...
export function CompareFloat() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-zinc-900/90 backdrop-blur-md border border-zinc-800 rounded-full p-2 pl-4 md:pl-6 flex items-center gap-4 md:gap-6 shadow-2xl shadow-white/5">
          <div className="flex items-center gap-3 md:gap-4">
            <span className="font-mono text-white font-bold text-xs md:text-sm whitespace-nowrap">
              ...
              {selectedMotors.map((motor) => (
                <div key={motor.id} className="relative group">
                  ...
                  <button
                    ...
                    className="absolute -top-1 -right-1 bg-zinc-700 rounded-full p-0.5 text-white hover:bg-zinc-600 md:opacity-0 md:group-hover:opacity-100 transition-all"
                  >
                    <X size={10} />
                  </button>
                </div>
          ...
          {selectedMotors.length >= 2 ? (
            <Link
              href="/compare"
              className="bg-white hover:bg-zinc-200 text-black px-4 md:px-6 py-2 rounded-full font-display font-medium text-xs md:text-sm uppercase flex items-center gap-2 transition-colors whitespace-nowrap"
            >
