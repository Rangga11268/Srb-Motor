...
export default function ComparePage() {
    ...
    return (
      <main className="min-h-screen bg-black text-white">
        ...
        <div className="h-screen flex flex-col items-center justify-center p-6 text-center">
          ...
          <h1 className="font-display font-medium text-2xl uppercase mb-2">
          ...
          <Link
            href="/gallery"
            className="bg-white text-black px-6 py-3 rounded-full font-bold font-mono tracking-wider uppercase hover:bg-zinc-200 transition-colors"
          >
            Kembali ke Galeri
          </Link>
        </div>
  ...
  const allSpecLabels = Array.from(
    new Set(
      selectedMotors.flatMap((m) => m.specifications.map((s) => s.label)),
    ),
  );

  return (
    <main className="min-h-screen bg-black text-white pt-36 md:pt-32 pb-40">
      <Navbar />
      ...
      <div className="container mx-auto px-6 mb-8 md:mb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-display font-medium text-2xl md:text-5xl uppercase tracking-tight">
              Komparasi <span className="text-zinc-500">Unit</span>
            </h1>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-[var(--mobile-cols)] md:grid-cols-[var(--desktop-cols)] gap-0 border-b border-zinc-800 pb-4 mb-4 md:pb-8 md:mb-8 sticky top-0 bg-black z-30">
                ...
                <div className="pt-12 md:pt-20 sticky left-0 bg-black z-40 border-r border-zinc-800 pr-2 md:pr-8 flex flex-col justify-end pb-2 md:pb-0 shadow-[4px_0_12px_rgba(0,0,0,0.8)] md:shadow-none">
                  <span className="font-mono text-white font-bold block mb-1 text-[9px] md:text-sm tracking-widest leading-tight px-2 md:px-0">
                    // UNIT
                  </span>
                ...
                {selectedMotors.map((motor) => (
                  ...
                  >
                    <button
                      ...
                      className="absolute top-2 right-1 md:right-6 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-full p-1.5 transition-all z-10 backdrop-blur-md"
                      title="Hapus"
                    >
                      <X size={12} />
                    </button>
                    <div className="h-full flex flex-col items-center text-center pt-6 md:pt-8 content-end">
                      ...
                      <div className="mt-auto w-full px-1">
                        <span className="font-mono text-[8px] md:text-xs text-zinc-400 font-bold mb-1 block tracking-widest uppercase truncate max-w-full">
                          {motor.category}
                        </span>
                        <h3 className="font-display font-medium text-[10px] md:text-2xl uppercase leading-none mb-1 md:mb-3 min-h-[2.5em] md:min-h-[2em] flex items-center justify-center line-clamp-2">
                          {motor.name}
                        </h3>
                        ...
                        <Link
                          href={`/motor/${motor.id}`}
                          className="inline-flex items-center justify-center gap-1 md:gap-2 text-[8px] md:text-xs font-mono font-bold uppercase tracking-wider text-zinc-500 hover:text-white transition-colors border border-zinc-800 hover:border-white px-2 py-1 md:px-4 md:py-2 rounded-full w-full md:w-auto"
                        >
                          Detail{" "}
                ...
                {Array.from({ length: 3 - selectedMotors.length }).map(
                  (_, i) => (
                    ...
                    >
                      <div className="w-20 h-20 rounded-full border-2 border-dashed border-zinc-500 group-hover:border-white flex items-center justify-center mb-4 transition-colors">
                        <span className="text-4xl text-zinc-500 group-hover:text-white transition-colors">
                          +
                        </span>
                      </div>
                      <span className="font-mono text-sm text-zinc-500 group-hover:text-white uppercase transition-colors">
                        Tambah Unit
                      </span>
                    </Link>
                  ),
              ...
              <div className="space-y-0">
                ...
                <div className="grid grid-cols-[var(--mobile-cols)] md:grid-cols-[var(--desktop-cols)] gap-0 md:gap-0 py-4 md:py-6 border-b border-zinc-800 items-center hover:bg-zinc-900/10 transition-colors">
                  <div className="sticky left-0 bg-black z-20 border-r border-zinc-800 pr-2 md:pr-8 py-2 font-mono font-bold text-zinc-500 uppercase text-[9px] md:text-sm tracking-wider shadow-[4px_0_12px_rgba(0,0,0,0.8)] md:shadow-none px-2 md:px-0">
                    Highlights
                  </div>
                  {selectedMotors.map((motor) => (
                    <div
                ...
                {allSpecLabels.map((label, idx) => (
                  ...
                  >
                    <div className="sticky left-0 bg-black z-20 md:bg-transparent md:static border-r border-zinc-800 pr-2 md:pr-8 py-2 font-mono font-bold text-zinc-500 text-[9px] md:text-sm break-words group-hover/row:text-white transition-colors shadow-[4px_0_12px_rgba(0,0,0,0.8)] md:shadow-none h-full flex items-center px-2 md:px-0">
